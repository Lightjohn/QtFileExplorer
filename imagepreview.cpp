...
imagePreview::imagePreview(QWidget *parent) : QWidget(parent, Qt::Window) {
...
void imagePreview::updateImage(QPixmap im) {
  int height = (static_cast<myWindows *>(this->parent()))->screenH;
  int width = (static_cast<myWindows *>(this->parent()))->screenW;
...
// This function is called when we want to have the opposite effect as the
// actual one Hided -> Shown Shown -> Hided
...
// Same idea that hidePreview: we got from fullScreen to normal or vice versa
...
void imagePreview::closeEvent(QCloseEvent *) { showing = false; }
